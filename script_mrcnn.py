...
# os.environ['CUDA_VISIBLE_DIVICES']="4"
...
if yn_dark_nuclei:
    ...
    for id_img in data_train:
        ...
        if 'class' in data_train[id_img]:
            data_train_dark_nuclei[id_img]['class'] = data_train[id_img]['class']
...
if yn_create_train_seg:
    if yn_use_aug:
        ...
        for id_img in data_train.keys():
            ...
            for i in range(4):
                data_train_aug[id_img + '_rot{}'.format(i)] = {}
                data_train_aug[id_img + '_rot{}'.format(i)]['image'] = image_cur
                data_train_aug[id_img + '_rot{}'.format(i)]['mask'] = mask_cur
                if 'class' in data_train[id_img]:
                    data_train_aug[id_img + '_rot{}'.format(i)]['class'] = data_train[id_img]['class']
...
class NucleiConfig(mrcnn.config.Config):
    ...
    NUM_CLASSES = 1 + 2  # background + 2 foreground
